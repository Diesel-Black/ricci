...
--   - Compute metric tensor and inverse; compute Gregorio and scalar curvature
...
-- Complexity: O(n^4).
CREATE OR REPLACE FUNCTION ricci.compute_christoffel_symbols(
...
BEGIN
    ...
    g_inv := ricci.compute_metric_inverse(metric_components, dimension);
...
CREATE OR REPLACE FUNCTION ricci.covariant_derivative(
...
BEGIN
    ...
    arr_field := ricci.vector_to_real_array(field_components);
...
CREATE OR REPLACE FUNCTION ricci.compute_metric_tensor_from_semantic_field(
...
BEGIN
    ...
    IF array_length(neighboring_fields, 1) >= 2 THEN
        arr_nb1 := ricci.vector_to_real_array(neighboring_fields[1]);
        arr_nb2 := ricci.vector_to_real_array(neighboring_fields[2]);
...
-- Complexity: O(n^3).
CREATE OR REPLACE FUNCTION ricci.compute_metric_inverse(
    metric_components FLOAT[],
    dimension INTEGER DEFAULT 100
) RETURNS FLOAT[] LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    det_g := ricci.matrix_determinant(metric_matrix, n);
    ...
    inverse_matrix := ricci.matrix_inverse_gauss_jordan(metric_matrix, n);
...
-- Complexity: O(n^3).
CREATE OR REPLACE FUNCTION ricci.matrix_determinant(
    matrix FLOAT[][],
    n INTEGER
) RETURNS FLOAT LANGUAGE plpgsql AS $$
DECLARE
...
-- Complexity: O(n^3).
CREATE OR REPLACE FUNCTION ricci.matrix_inverse_gauss_jordan(
    matrix FLOAT[][],
    n INTEGER
) RETURNS FLOAT[][] LANGUAGE plpgsql AS $$
DECLARE
...
-- Summary: Compute Gregorio curvature components from Γ and ∂Γ contractions.
-- Inputs:
...
-- Returns: FLOAT[] — flattened Gregorio components R_ij (length n^2).
-- Complexity: O(n^4).
CREATE OR REPLACE FUNCTION ricci.compute_gregorio_curvature(
    christoffel_symbols FLOAT[],
...
BEGIN
    gregorio_components := ARRAY(SELECT 0.0 FROM generate_series(1, n * n));
    ...
    FOR i IN 1..n LOOP
        FOR j IN 1..n LOOP
            ...
            idx := (i-1) * n + j;
            gregorio_components[idx] := R_ij;
        END LOOP;
    END LOOP;
    
    RETURN gregorio_components;
END;
$$;

-- Summary: Contract scalar curvature R = g^{ij} R_{ij} using flattened arrays.
-- Inputs:
--   - gregorio_components FLOAT[] — flattened R_{ij}
--   - metric_inverse FLOAT[] — flattened g^{ij}
--   - dimension INTEGER — active dimension n (default 100)
-- Returns: FLOAT — scalar curvature R.
CREATE OR REPLACE FUNCTION ricci.compute_scalar_curvature(
    gregorio_components FLOAT[],
    ...
    WITH curvature_calc AS (
        SELECT 
            sum(
                ...
                gregorio_components[(i-1)*dimension + j]
            ) as scalar_R
...
CREATE OR REPLACE FUNCTION ricci.integrate_geodesic_distance(
    point_a UUID,
    point_b UUID,
...
BEGIN
    ...
    arr_pa := ricci.vector_to_real_array(pa_coords);
    arr_pb := ricci.vector_to_real_array(pb_coords);
...
-- Complexity: O(n^3).
CREATE OR REPLACE FUNCTION ricci.compute_recursive_coupling_tensor(
    point_p UUID,
    point_q UUID,
...
BEGIN
    ...
    FROM ricci.manifold_points WHERE id = point_p;
    
    SELECT semantic_field
    INTO semantic_q
    FROM ricci.manifold_points WHERE id = point_q;
    ...
    DECLARE
        arr_p FLOAT[] := ricci.vector_to_real_array(semantic_p);
        arr_q FLOAT[] := ricci.vector_to_real_array(semantic_q);
        arr_c FLOAT[] := ricci.vector_to_real_array(coherence_p);
    BEGIN
        FOR i IN 1..dim LOOP
            FOR j IN 1..dim LOOP
                FOR k IN 1..dim LOOP
