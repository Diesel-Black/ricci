...
CREATE OR REPLACE FUNCTION ricci.detect_coordination_via_coupling(
    ...
    WITH coupling_analysis AS (
        ...
        FROM ricci.manifold_points mp1
        JOIN ricci.recursive_coupling rc ON mp1.id = rc.point_p
...
-- Numerical guards: Ensure time deltas ≥ 1s; default intervention baseline when circumspection not low.
-- Returns: TABLE(point_id, coherence_acceleration, semantic_curvature, escalation_trajectory, intervention_urgency).
CREATE OR REPLACE FUNCTION ricci.detect_escalation_via_field_evolution(
...
BEGIN
    FOR point_rec IN 
        SELECT * FROM ricci.manifold_points 
            ...
            RETURN QUERY SELECT 
                ...
                CASE 
                    WHEN coherence_accel > 0.3 AND EXISTS(
                        SELECT 1 FROM ricci.sapience_field sf 
                        WHERE sf.point_id = point_rec.id 
                        AND sf.circumspection_factor < 0.3
                    )
...
-- Terms: dalembertian (covariant second derivative + Gregorio term), stability attractor, autopoietic gradient, circumspection damping.
...
CREATE OR REPLACE FUNCTION ricci.evolve_coherence_field_complete(
    point_id UUID,
...
DECLARE
    ...
    arr_circumspection_constraint FLOAT[];
    
    -- Geometric computation variables
    second_covariant_deriv FLOAT;
    gregorio_coupling_term FLOAT;
...
BEGIN
    ...
    arr_current := ricci.vector_to_real_array(current_coherence);
    arr_len := COALESCE(array_length(arr_current, 1), 0);
    n := LEAST(dim, GREATEST(arr_len, 1));

    -- Compute metric inverse
    metric_inverse := ricci.compute_metric_inverse(metric_tensor, dim);
    ...
    coherence_mag := COALESCE(
        ...
        COALESCE(ricci.vector_l2_norm(current_coherence), 0.0)
    );
    ...
    arr_circumspection_constraint := ARRAY(SELECT 0.0::REAL FROM generate_series(1, 2000));
    ...
    FOR i IN 1..LEAST(dim, 2000) LOOP
        ...
        gregorio_coupling_term := - semantic_mass * arr_current[LEAST(i, GREATEST(arr_len, 1))];
        arr_dalembertian[i] := second_covariant_total + gregorio_coupling_term;
    END LOOP;
    ...
    -- Circumspection constraint damping
    FOR i IN 1..LEAST(dim, 2000) LOOP
        val_i := arr_current[LEAST(i, GREATEST(arr_len, 1))];
        arr_circumspection_constraint[i] := -0.1 * coherence_mag * val_i;
    END LOOP;
    ...
    FOR i IN 1..LEAST(dim, 2000) LOOP
        ...
        arr_new[i] := 
            val_i + dt * (
                arr_dalembertian[i] + arr_attractor_gradient[i] + arr_autopoietic_gradient[i] + arr_circumspection_constraint[i]
            );
    END LOOP;
...
CREATE OR REPLACE FUNCTION ricci.detect_all_signatures(
    point_id UUID
) RETURNS TABLE(
    signature_type TEXT,
    severity FLOAT,
...
CREATE OR REPLACE VIEW ricci.coordination_alerts AS
SELECT 
    cluster_id,
    cluster_size,
...
DO $$ BEGIN
    PERFORM 1 FROM pg_matviews WHERE schemaname = 'ricci' AND matviewname = 'geometric_alerts_mv';
    IF FOUND THEN
        EXECUTE 'DROP MATERIALIZED VIEW IF EXISTS ricci.geometric_alerts_mv';
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_geometric_alerts_mv_point_time
    ON ricci.geometric_alerts_mv(point_id, creation_timestamp);
CREATE INDEX IF NOT EXISTS idx_geometric_alerts_mv_severity_time
    ON ricci.geometric_alerts_mv(severity, creation_timestamp);
CREATE INDEX IF NOT EXISTS idx_geometric_alerts_mv_priority
    ON ricci.geometric_alerts_mv(priority);

CREATE OR REPLACE FUNCTION ricci.refresh_geometric_alerts()
RETURNS void LANGUAGE SQL AS $$
    REFRESH MATERIALIZED VIEW ricci.geometric_alerts_mv;
$$;

-- Performance optimization indexes

CREATE INDEX IF NOT EXISTS idx_manifold_points_semantic_field
    ON ricci.manifold_points USING hnsw (semantic_field vector_cosine_ops);

CREATE INDEX IF NOT EXISTS idx_manifold_points_coherence_field
    ON ricci.manifold_points USING hnsw (coherence_field vector_cosine_ops);

CREATE INDEX IF NOT EXISTS idx_manifold_points_semantic_mass
    ON ricci.manifold_points(semantic_mass, creation_timestamp);

CREATE INDEX IF NOT EXISTS idx_manifold_points_user_timestamp
    ON ricci.manifold_points(user_fingerprint, creation_timestamp);

CREATE INDEX IF NOT EXISTS idx_recursive_coupling_magnitude
    ON ricci.recursive_coupling(coupling_magnitude, computed_at);

CREATE INDEX IF NOT EXISTS idx_recursive_coupling_points
    ON ricci.recursive_coupling(point_p, point_q, computed_at);

CREATE INDEX IF NOT EXISTS idx_sapience_field_values
    ON ricci.sapience_field(sapience_value, circumspection_factor); 