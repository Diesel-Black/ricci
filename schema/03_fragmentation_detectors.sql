...
-- File: schema/03_fragmentation_detectors.sql
--
...
BEGIN
    ...
    IF total_samples > 2 THEN
        ...
        autopoietic_generation_rate := GREATEST(0.0, 
            ricci.compute_autopoietic_potential(coherence_mag) * 
...
BEGIN
    ...
    coherence_mag := COALESCE(ricci.vector_l2_norm(current_coherence), 0.0);
    
    SELECT * INTO derivatives FROM ricci.compute_finite_differences(point_id);
...
--   - sapience_compensation_threshold FLOAT — τ (default 0.3)
-- Assumptions: Use last 10 couplings where point is p or q; compensate by sapience×circumspection.
...
BEGIN
    ...
    IF sample_count > 1 THEN
        ...
        compensatory_sapience := COALESCE(current_sapience, 0.0) * COALESCE(circumspection_factor, 0.5);
        
        IF coupling_decay_rate < decay_threshold AND 
           compensatory_sapience < sapience_compensation_threshold THEN
            ...
            RETURN QUERY SELECT 
                ...
                format(
                    'Coupling decay rate: %s < 0, compensatory sapience: %s < threshold (samples: %s)',
                    to_char(coupling_decay_rate::numeric, 'FM999990.000'),
                    to_char(compensatory_sapience::numeric, 'FM999990.000'),
                    sample_count
                );
        END IF;
    END IF;
...
CREATE OR REPLACE FUNCTION ricci.detect_fragmentation_signatures(
    point_id UUID
) RETURNS TABLE(
    signature_type TEXT,
    severity FLOAT,
...
CREATE OR REPLACE FUNCTION ricci.compute_finite_differences(
    point_id UUID,
...
BEGIN
    ...
    INTO current_coherence
    FROM ricci.manifold_points WHERE id = point_id;
