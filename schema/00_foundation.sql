...
-- Purpose: Establish the ricci schema, vector extension, entities, and math utilities.
-- Exposes:
--   - Define core entities: manifold points, recursive coupling, sapience field
--   - Compute semantic mass, autopoietic potential, circumspection operator
...
CREATE SCHEMA IF NOT EXISTS ricci;
...
CREATE TABLE IF NOT EXISTS ricci.manifold_points (
    ...
    -- Compute semantic mass components: M = A · ρ · D
...
CREATE TABLE IF NOT EXISTS ricci.recursive_coupling (
    id UUID PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS ricci.sapience_field (
    ...
    -- Store circumspection operators
...
-- Returns: FLOAT — semantic mass M = A · ρ · D ∈ [0, ∞).
CREATE OR REPLACE FUNCTION ricci.compute_semantic_mass(
    recursive_depth FLOAT,
    metric_determinant FLOAT,
    attractor_stability FLOAT
...
CREATE OR REPLACE FUNCTION ricci.compute_autopoietic_potential(
    coherence_magnitude FLOAT,
...
-- Summary: Compute circumspection damping against excessive recursion from coupling magnitude.
-- Inputs:
...
CREATE OR REPLACE FUNCTION ricci.compute_circumspection_operator(
    coupling_magnitude FLOAT,
...
CREATE OR REPLACE FUNCTION ricci.vector_to_real_array(v vector)
RETURNS FLOAT[] LANGUAGE SQL AS $$
    SELECT ARRAY(
        SELECT trim(x)::FLOAT
...
CREATE OR REPLACE FUNCTION ricci.get_active_dimension()
RETURNS INTEGER LANGUAGE SQL AS $$
    SELECT 100
$$;

CREATE OR REPLACE FUNCTION ricci.get_small_window()
RETURNS INTEGER LANGUAGE SQL AS $$
    SELECT 50
$$;
...
CREATE OR REPLACE FUNCTION ricci.vector_l2_norm(v vector)
RETURNS FLOAT LANGUAGE SQL AS $$
    WITH elems AS (
...
-- Summary: Compute L2 norm of the first n components of a pgvector.
-- Inputs:
--   - v vector — pgvector input
--   - n integer — number of leading dimensions to include
-- Returns: FLOAT — Euclidean norm over first n components.
CREATE OR REPLACE FUNCTION ricci.vector_l2_norm_first_n(v vector, n integer)
RETURNS FLOAT LANGUAGE SQL AS $$
    WITH arr AS (
        SELECT ricci.vector_to_real_array(v) AS a,
               COALESCE(array_length(ricci.vector_to_real_array(v), 1), 0) AS len
    )
    SELECT sqrt(
        sum(
            pow(
                COALESCE(a[LEAST(i, GREATEST(len, 1))], 0.0),
                2
            )
        )
    )
    FROM generate_series(1, n) AS i, arr;
$$;

-- Summary: Compute L2 distance between two vectors using only the first n components.
-- Inputs:
--   - v1 vector, v2 vector — pgvector inputs
--   - n integer — number of leading dimensions to include
-- Returns: FLOAT — Euclidean distance over first n components.
CREATE OR REPLACE FUNCTION ricci.vector_l2_distance_first_n(v1 vector, v2 vector, n integer)
RETURNS FLOAT LANGUAGE SQL AS $$
    WITH a AS (
        SELECT ricci.vector_to_real_array(v1) AS arr,
               COALESCE(array_length(ricci.vector_to_real_array(v1), 1), 0) AS len
    ),
    b AS (
        SELECT ricci.vector_to_real_array(v2) AS arr,
               COALESCE(array_length(ricci.vector_to_real_array(v2), 1), 0) AS len
    )
    SELECT sqrt(
        sum(
            pow(
                COALESCE(a.arr[LEAST(i, GREATEST(a.len, 1))], 0.0) -
                COALESCE(b.arr[LEAST(i, GREATEST(b.len, 1))], 0.0),
                2
            )
        )
    )
    FROM generate_series(1, n) AS i, a, b;
$$;