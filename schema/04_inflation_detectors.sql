...
-- Exposes:
--   - Detect Boundary Hyperasymmetry: local mass grows while ecological mass drains
--   - Detect Field Hypercoherence: coherence saturation with low external influence flux
--   - Detect Structure Hyperexpansion: autopoietic dominance with low circumspection and sapience
-- Conventions:
...
CREATE OR REPLACE FUNCTION ricci.detect_boundary_hyperasymmetry(
    point_id UUID,
...
CREATE OR REPLACE FUNCTION ricci.detect_field_hypercoherence(
    point_id UUID,
...
CREATE OR REPLACE FUNCTION ricci.detect_structure_hyperexpansion(
    point_id UUID,
...
BEGIN
    ...
    WHERE sf.point_id = detect_structure_hyperexpansion.point_id
...
BEGIN
    RETURN QUERY SELECT * FROM ricci.detect_boundary_hyperasymmetry(point_id);
    RETURN QUERY SELECT * FROM ricci.detect_field_hypercoherence(point_id);
    RETURN QUERY SELECT * FROM ricci.detect_structure_hyperexpansion(point_id);
    RETURN;
END;
$$; 