-- RICCI: Performance Tests
-- File: tests/performance/test_function_performance.sql
--
-- Performance benchmarks for RICCI functions
...
SELECT ricci_test.assert_performance(
    ...
    'SELECT ricci.compute_semantic_mass(generate_series(1,1000), random(), random()) FROM generate_series(1,10)',
    '1 second',
    'performance'
);
...
SELECT ricci_test.assert_performance(
    ...
    'SELECT ricci.compute_autopoietic_potential(random()) FROM generate_series(1,1000)',
    '1 second',
    'performance'
);

\echo 'Performance: circumspection_operator (1k evals) ≤ 1s'
SELECT ricci_test.assert_performance(
    'circumspection_operator_performance',
    'SELECT ricci.compute_circumspection_operator(random()) FROM generate_series(1,1000)',
    '1 second',
    'performance'
);
...
SELECT ricci_test.assert_performance(
    ...
    'SELECT ricci.matrix_determinant(
...
BEGIN
    ...
    PERFORM ricci_test.assert_performance(
        ...
        format('SELECT * FROM ricci.detect_all_signatures(''%s'')', test_point),
        '10 seconds',
        'performance'
    );
...
BEGIN
    ...
    FOR i IN 1..5 LOOP
        test_point := ricci_test.create_test_manifold_point(NULL, NULL, 'test_user_' || i);
    ...
    PERFORM ricci_test.assert_performance(
        ...
        'SELECT * FROM ricci.detect_coordination_via_coupling(''1 hour'', 0.7, 2)',
        '5 seconds',
        'performance'
    );
...
BEGIN
    ...
    PERFORM ricci_test.assert_performance(
        ...
        format('SELECT ricci.evolve_coherence_field_complete(''%s'', 0.01)', test_point),
        '8 seconds',
        'performance'
    );
...
SELECT ricci_test.assert_performance(
    ...
    'SELECT ricci_test.create_test_manifold_point() FROM generate_series(1,20)',
    '5 seconds',
    'performance'
);

SELECT ricci_test.cleanup_test_data();
...
BEGIN
    ...
    PERFORM ricci_test.assert_performance(
        ...
        format('SELECT id FROM ricci.manifold_points 
                WHERE semantic_field IS NOT NULL 
                ORDER BY semantic_field <-> (SELECT semantic_field FROM ricci.manifold_points WHERE id = ''%s'') 
