...
-- Create test schema
CREATE SCHEMA IF NOT EXISTS ricci_test;
...
CREATE TABLE IF NOT EXISTS ricci_test.test_results (
...
TRUNCATE ricci_test.test_results;
...
-- Returns: boolean; inserts a row into ricci_test.test_results.
CREATE OR REPLACE FUNCTION ricci_test.assert_float_equals(
    test_name TEXT,
    expected_val FLOAT,
    actual_val FLOAT,
...
BEGIN
    ...
    INSERT INTO ricci_test.test_results (
...
-- Returns: boolean; inserts a row into ricci_test.test_results.
CREATE OR REPLACE FUNCTION ricci_test.assert_true(
    test_name TEXT,
    condition BOOLEAN,
    category TEXT DEFAULT 'general',
...
-- Returns: boolean; inserts a row into ricci_test.test_results.
CREATE OR REPLACE FUNCTION ricci_test.assert_array_equals(
    test_name TEXT,
...
-- Returns: boolean; inserts a row into ricci_test.test_results.
CREATE OR REPLACE FUNCTION ricci_test.assert_no_error(
    test_name TEXT,
    sql_statement TEXT,
...
-- Returns: boolean; inserts a row into ricci_test.test_results with execution_time=duration.
CREATE OR REPLACE FUNCTION ricci_test.assert_performance(
    test_name TEXT,
    sql_statement TEXT,
...
BEGIN
    ...
    -- Child rows
    DELETE FROM ricci.recursive_coupling 
    WHERE point_p = ANY(test_point_ids) OR point_q = ANY(test_point_ids);

    DELETE FROM ricci.sapience_field
    WHERE point_id = ANY(test_point_ids);

    -- Parent rows
    DELETE FROM ricci.manifold_points 
    WHERE id = ANY(test_point_ids);
END;
$$;

CREATE OR REPLACE FUNCTION ricci_test.teardown_test_framework(drop_schema BOOLEAN DEFAULT false)
...
BEGIN
    ...
    IF drop_schema THEN
        EXECUTE 'DROP SCHEMA IF EXISTS ricci_test CASCADE';
    END IF;
END;
$$;
